...
def view_results(X_train, y_train, X_test, y_test, model):
    try:
        ...
        print('Import error on: helper functions')
    plt.figure(figsize=(12,6))
    plt.subplot(1,2,1)
    plt.title("Train")
...
class Timer:
    ...
    def interval(self, name:str = 'Interval N'):
        ...
        values_of_last = sum([el[1] for el in self.since_last])
        self.since_last.append([name, interval_value-values_of_last])
    ...
    def show_as_print(self, decimals: tuple = (2,2,2)):
        ...
        args:
            decimals: Changes the amount of decimals shown for (total time,  interval time,  since start time)"""
        start_decimals = f'.{decimals[0]}f'
        interval_decimals = f'.{decimals[1]}f'
        since_decimals = f'.{decimals[2]}f'
        ...
        if self.since_last != [['start', self.starttimer]]:
            ...
            since_last_display = [[name,f"{number:{interval_decimals}}"] for name,number in self.since_last]
